...
from assets.mappings import METADATA_TO_DUBLIN_XML_MAPPING, DOMAIN_TO_COLLECTION_MAPPING, METADATA_TO_DSPACE_XML_MAPPING, METADATA_TO_LOCAL_XML_MAPPING, METADATA_TO_PROJECT_XML_MAPPING, DOMAIN_TO_THUMBNAIL_FILE_MAPPING
...
def bulk_deposit(excel_path, csv_base_path, results_path):
    ...
    for index, row in df.iterrows():
        ...
        # generate metadata_project.xml
        root = ET.Element("dublin_core", schema="project")
        for key, value in METADATA_TO_PROJECT_XML_MAPPING.items():
            element = value['element']
            qualifier = value['qualifier']
            text_value = row_dict[key]
            dcvalue = ET.SubElement(root, "dcvalue")
        ...
        tree = ET.ElementTree(root)
        ET.indent(tree)
        tree.write(os.path.join(curr_result_path, "metadata_project.xml"), encoding="UTF-8", xml_declaration=True)
